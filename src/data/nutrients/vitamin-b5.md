---
name: Vitamin B5
altName: Pantothenic Acid
symbol: B5
category: vitamin
subcategory: water-soluble
essentiality: essential
sortOrder: 9
description: Component of Coenzyme A (CoA), critical for fatty acid synthesis and oxidation, and the citric acid cycle. Found in virtually all foods.
functions:
  - Component of Coenzyme A — central to all energy metabolism
  - Required for fatty acid synthesis and oxidation
  - Needed for synthesis of cholesterol, steroid hormones, and neurotransmitters
  - Supports wound healing and skin health
rda:
  nih:
    - group: Adults
      amount: 5 mg (AI)
    - group: Pregnant
      amount: 6 mg
    - group: Lactating
      amount: 7 mg
  who:
    - group: Adults
      amount: 5 mg
  efsa:
    - group: Adults
      amount: 5 mg
deficiency:
  symptoms:
    early:
      - "Fatigue"
      - "Irritability"
      - "Sleep disturbance"
      - "Numbness in feet"
    moderate:
      - "Burning feet syndrome (paresthesia)"
      - "GI disturbances (nausea, cramping)"
      - "Headache"
      - "Restlessness"
    severe:
      - "Severe burning feet syndrome"
      - "Adrenal insufficiency"
      - "Severe fatigue and weakness"
    longTerm:
      - "Deficiency is extremely rare due to widespread availability in food"
      - "Chronic adrenal dysfunction (theoretical)"
      - "Impaired CoA-dependent metabolic processes"
  conditions:
    - Burning feet syndrome (historically seen in WWII prisoners of war)
  riskGroups:
    - Severe malnutrition only
toxicity:
  symptoms:
    - Diarrhea at very high doses (10+ g/day)
  upperLimit: No established UL
  notes: One of the safest vitamins. Very high doses may cause GI upset.
normalLevels:
  bloodRange: 1.6-2.7 µmol/L (whole blood)
  unit: µmol/L
testing:
  methods:
    - Whole blood pantothenic acid
    - Urinary pantothenic acid excretion
  sampleType: Blood/Urine
sources:
  - name: Beef liver (3 oz)
    amount: 8.3
    unit: mg
  - name: Chicken breast (3 oz)
    amount: 1.3
    unit: mg
  - name: Avocado (1 whole)
    amount: 2.0
    unit: mg
  - name: Sunflower seeds (1 oz)
    amount: 2.0
    unit: mg
  - name: Mushrooms (½ cup)
    amount: 1.3
    unit: mg
  - name: Sweet potato (1 medium)
    amount: 1.0
    unit: mg
myths:
  - myth: You need B5 supplements for acne
    reality: While some studies suggest mega-dose pantothenic acid (5-10g/day) may reduce acne, the evidence is limited and such high doses are unnecessary for most people.
precautions:
  - Food processing reduces B5 content by 20-70%
  - Freezing and canning significantly reduce pantothenic acid levels
interactions:
  - substance: Biotin
    effect: B5 and biotin may compete for absorption at high supplemental doses
history: Discovered in 1931 by Roger J. Williams. Named from Greek "pantos" meaning "everywhere" because it's found in nearly all foods. Williams later became a major proponent of nutritional individuality, arguing that every person has unique nutritional needs.
funFacts:
  - The name means "from everywhere" — deficiency is almost impossible on a normal diet
  - Royal jelly (from honeybees) is one of the richest natural sources of B5
  - Dexpanthenol (provitamin B5) is widely used in skin care products and wound healing ointments
enzymeRoles:
  - "Component of Coenzyme A (CoA) — required by 4% of all known enzymes"
  - "Component of acyl carrier protein (ACP) in fatty acid synthase"
hormonalRoles:
  - "Required for steroid hormone synthesis (cortisol, sex hormones via CoA)"
  - "Supports melatonin synthesis"
  - "Required for acetylcholine production"
metabolicPathways:
  - "Coenzyme A is central to TCA cycle (acetyl-CoA entry)"
  - "Fatty acid synthesis and beta-oxidation"
  - "Cholesterol and steroid synthesis (mevalonate pathway)"
  - "Amino acid metabolism"
organsAffected:
  - "Adrenal glands (steroid synthesis)"
  - "Liver"
  - "All metabolically active tissues"
cellularFunctions:
  - "Acyl group transfer reactions"
  - "Energy production from all macronutrients"
  - "Protein acetylation (histone modification)"
  - "Lipid metabolism"
---
