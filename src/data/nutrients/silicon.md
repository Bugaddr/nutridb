---
name: Silicon
altName: Silicic Acid (bioavailable form)
symbol: Si
category: other
subcategory: non-essential
essentiality: non-essential
sortOrder: 35
description: The third most abundant trace element in the body. Believed to support collagen synthesis, bone mineralization, and connective tissue health. Not yet classified as essential.
functions:
  - Supports collagen synthesis and crosslinking
  - Bone mineralization and formation
  - Connective tissue integrity (cartilage, tendons)
  - Skin, hair, and nail health
  - May support arterial wall integrity
rda:
  nih:
    - group: No RDA established
      amount: Estimated intake 20-50 mg/day from food
  who:
    - group: No recommendation
      amount: N/A
  efsa:
    - group: No recommendation
      amount: N/A
deficiency:
  symptoms:
    early:
      - "Brittle nails"
      - "Dry hair"
      - "Slow wound healing"
    moderate:
      - "Weakened connective tissue"
      - "Poor bone mineralization"
      - "Skin elasticity loss"
    severe:
      - "Not well-characterized in humans"
      - "Animal models: skeletal deformities, reduced collagen content"
    longTerm:
      - "Osteoporosis risk"
      - "Poor connective tissue integrity"
      - "Arterial stiffness"
  riskGroups:
    - N/A
toxicity:
  symptoms:
    - Dietary silicon is considered very safe
    - Silicosis (lung disease) is from inhaling crystalline silica dust — different form
  upperLimit: No established UL for dietary silicon
normalLevels:
  bloodRange: Not routinely measured
  unit: N/A
testing:
  methods:
    - Not routinely tested
  sampleType: N/A
sources:
  - name: Beer (12 oz)
    amount: 6.4-56
    unit: mg (varies by type)
  - name: Bananas (1 medium)
    amount: 5.4
    unit: mg
  - name: Green beans (1 cup)
    amount: 6.1
    unit: mg
  - name: Brown rice (1 cup)
    amount: 4.5
    unit: mg
  - name: Mineral water (1 L)
    amount: 0-40
    unit: mg (varies)
myths:
  - myth: Silicon supplements dramatically improve hair and nails
    reality: While silicon plays a role in connective tissue, clinical evidence for supplement benefits in people with adequate diets is limited. Most claims are marketing-driven.
precautions:
  - Orthosilicic acid is the bioavailable form — most dietary silicon must be converted
  - Silicosis from inhaling quartz dust is a serious occupational hazard — unrelated to dietary silicon
interactions:
  - substance: Aluminum
    effect: Silicon may reduce aluminum absorption — potentially protective
history: Silicon's biological role was first proposed in the 1970s by Edith Carlisle and Klaus Schwarz, who showed silicon-deprived chicks had skeletal abnormalities. Its status in human nutrition remains provisional.
funFacts:
  - Beer is actually one of the richest dietary sources of bioavailable silicon
  - Silicon is the second most abundant element in Earth's crust after oxygen
  - Diatoms (single-celled algae) build their shells entirely from silicon — they create 20% of Earth's oxygen
enzymeRoles:
  - "May activate prolyl hydroxylase (collagen synthesis)"
  - "Possible involvement in glycosaminoglycan synthesis enzymes"
hormonalRoles:
  - "No established direct hormonal roles"
  - "May influence connective tissue growth factors"
metabolicPathways:
  - "Collagen crosslinking"
  - "Glycosaminoglycan synthesis"
  - "Bone mineralization (hydroxyapatite nucleation)"
organsAffected:
  - "Connective tissue (skin, tendons, cartilage)"
  - "Bones"
  - "Blood vessels (aorta — high silicon content)"
  - "Hair and nails"
cellularFunctions:
  - "Collagen fiber maturation"
  - "Extracellular matrix formation"
  - "Bone matrix calcification initiation"
supplementForms:
  - form: "Orthosilicic acid (OSA)"
    bioavailability: "High"
    notes: "Monomeric form, most bioavailable"
  - form: "Choline-stabilized orthosilicic acid (ch-OSA)"
    bioavailability: "High"
    notes: "Stabilized with choline, patented (BioSil), well studied"
  - form: "Colloidal silicon dioxide"
    bioavailability: "Low"
    notes: "Poorly absorbed, mainly used as excipient"
  - form: "Horsetail extract"
    bioavailability: "Low"
    notes: "Natural source, silicon bound in plant matrix, poor absorption"
---
