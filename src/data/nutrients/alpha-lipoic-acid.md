---
name: Alpha-Lipoic Acid
altName: ALA / Thioctic Acid
symbol: ALA
category: other
subcategory: non-essential
essentiality: non-essential
sortOrder: 33
description: A sulfur-containing compound that functions as a cofactor in energy metabolism and as a unique antioxidant — active in both water and fat environments.
functions:
  - Cofactor for mitochondrial enzyme complexes (pyruvate dehydrogenase, alpha-ketoglutarate dehydrogenase)
  - Universal antioxidant (works in both aqueous and lipid environments)
  - Regenerates other antioxidants (vitamins C, E, glutathione)
  - Chelates heavy metals
  - Improves insulin sensitivity
rda:
  nih:
    - group: No RDA established
      amount: Body synthesizes sufficient amounts; supplemental doses typically 300-600 mg/day
  who:
    - group: No recommendation
      amount: N/A
  efsa:
    - group: No recommendation
      amount: N/A
deficiency:
  symptoms:
    early:
      - "Increased oxidative stress markers"
      - "Mild fatigue"
    moderate:
      - "Body synthesizes ALA, so dietary deficiency is not well-documented"
      - "Theoretically: decreased antioxidant capacity, increased oxidative damage"
    severe:
      - "True deficiency not documented in humans (body synthesizes it)"
      - "Lipoic acid synthase defects (genetic) cause severe metabolic disease"
    longTerm:
      - "Genetic deficiency: lactic acidosis, seizures, developmental regression"
      - "Theoretical: accelerated aging from reduced antioxidant recycling"
  riskGroups:
    - N/A
toxicity:
  symptoms:
    - GI upset, skin rash
    - Low blood sugar (can potentiate diabetes medications)
  upperLimit: No established UL. Doses of 600-1800 mg/day used in clinical trials.
normalLevels:
  bloodRange: Not routinely measured clinically
  unit: N/A
testing:
  methods:
    - Not routinely tested
  sampleType: N/A
sources:
  - name: Beef kidney (3 oz)
    amount: 0.32
    unit: mg
  - name: Spinach (1 cup)
    amount: 0.13
    unit: mg
  - name: Broccoli (1 cup)
    amount: 0.11
    unit: mg
  - name: Tomatoes (1 cup)
    amount: 0.06
    unit: mg
myths:
  - myth: ALA is a powerful weight loss supplement
    reality: While some studies show modest effects on weight (1-2 kg), it's not a significant weight loss agent. Its primary evidence-based use is for diabetic neuropathy.
precautions:
  - Can lower blood sugar — diabetics should monitor closely
  - R-lipoic acid (R-ALA) is the natural, more bioactive form vs racemic ALA in most supplements
  - Take on empty stomach for best absorption
interactions:
  - substance: Insulin and diabetes medications
    effect: May enhance blood sugar lowering — risk of hypoglycemia
  - substance: Thyroid medications
    effect: May lower thyroid hormone levels — monitor if supplementing
  - substance: Chemotherapy
    effect: Antioxidant effects may theoretically interfere (unproven)
history: Alpha-lipoic acid was discovered in 1951 by Lester Reed, who identified it as a cofactor for pyruvate dehydrogenase. Its antioxidant properties and potential therapeutic uses have been studied since the 1990s. It's an approved drug for diabetic neuropathy in Germany.
funFacts:
  - ALA is the only antioxidant that works in both water and fat — earning it the name "universal antioxidant"
  - It can regenerate other antioxidants (vitamin C, E, glutathione, CoQ10) — a recycler of recyclers
  - Germany has approved ALA as a prescription drug for diabetic neuropathy since the 1960s
enzymeRoles:
  - "Cofactor for pyruvate dehydrogenase complex"
  - "Cofactor for alpha-ketoglutarate dehydrogenase complex"
  - "Cofactor for branched-chain keto acid dehydrogenase"
hormonalRoles:
  - "Enhances insulin signaling (AMPK activation)"
  - "Activates AMPK (cellular energy sensor)"
metabolicPathways:
  - "Pyruvate dehydrogenase complex (pyruvate → acetyl-CoA)"
  - "TCA cycle (alpha-ketoglutarate step)"
  - "Branched-chain amino acid catabolism"
  - "Glutathione recycling"
organsAffected:
  - "Mitochondria (all tissues)"
  - "Liver"
  - "Peripheral nerves"
  - "Brain"
cellularFunctions:
  - "Mitochondrial energy metabolism"
  - "Universal antioxidant (both aqueous and lipid phases)"
  - "Antioxidant recycling (regenerates vitamin C, E, glutathione, CoQ10)"
  - "Heavy metal chelation"
  - "NF-κB modulation (anti-inflammatory)"
supplementForms:
  - form: "R-lipoic acid (R-ALA)"
    bioavailability: "High"
    notes: "Natural enantiomer, more biologically active, less stable"
  - form: "Racemic alpha-lipoic acid (R/S-ALA)"
    bioavailability: "Moderate"
    notes: "Standard supplement, 50% R-form + 50% S-form"
  - form: "Na-R-ALA (sodium R-lipoate)"
    bioavailability: "High"
    notes: "Stabilized R-form, better absorption and stability"
---
